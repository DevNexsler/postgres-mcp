#!/usr/bin/env bash
# Production-shaped gateway qualification for persisted-context identity (#2840).
set -Eeuo pipefail
...
compose=(docker compose --project-directory "$scratch/cds" -f "$scratch/cds/docker-compose.outbound-qualification.yml")
cleanup() {
  local result=$?
  trap - EXIT
  if (( result != 0 )); then
...
# Candidate image /app is the gateway archive; live probes live under /app/tests/live.
"${compose[@]}" exec -T "${exec_env[@]}" gateway python /app/tests/live/gateway_persisted_context_candidate.py
...
print(json.dumps({"source_sha": source, "cds_sha": cds, "image_id": inspect(image)[0]["Id"],
                  "containers": states, "qualification": "passed"}, sort_keys=True))
PY
