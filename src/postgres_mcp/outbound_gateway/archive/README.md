...
These env vars are **no longer read** by the gateway as of 2026-08-27. The gateway now gates on `FeaturePolicy.enabled_operations` (a single flat allowlist) and on `ALLOWED_COMBINATIONS` (role+operation+intent triplets, independent of inbound provider).
