...
These env vars are **no longer read** by the gateway as of 2026-08-27. Gateway request validation now depends on pydantic shape validation (enforces role+operation+intent match via `ExecuteRequest` model constraints) and the kill switch (`FeaturePolicy.kill_switch`) rather than runtime policy matrices. Fine-grained traffic control will be added in Task 6.
